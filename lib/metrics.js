...
function summariseHAR(har) {
    ...
    addContentLengthHeaders(har).then(function(har) {
        ...
        var outputSummary = {
            ...
            summary: {
                all: getResourceSummary(har.log.entries)
            }
        };

        // Do the same for external resources (ex: Frameworks, ORB, etc)
        _.each(CONFIG.assetGroups, function(match, groupName) {
            ...
            outputSummary.summary[groupName] = getResourceSummary(groupAssets);
        });
...
function getResourceSummary(entries) {
    ...
    entries.forEach(function(entry) {
        ...
        contentTypes[assetMeta.type] = contentTypes[assetMeta.type] || { size: 0, entries: [] };
