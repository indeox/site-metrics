...
function runMetrics(opts) {
    var screenName = (opts.disableJS) ? 'NoJS' : opts.viewport,
        urlSlug    = opts.url.replace(/[^a-z0-9]/gi, '_').toLowerCase(),
        screenshotFilename = now.format("YYYYMMDD_HHmmss") + '_' + urlSlug + '_' + screenName + '.jpg';

    return metrics.processPage(opts).then(function(result) {
        finalResults[opts.url][screenName] = result;
        finalResults[opts.url][screenName].screenName = screenName;
